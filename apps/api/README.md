...
```bash
# A. 只填元数据 → API 用 AI stub 生成占位 review.json
...
2. **SCORING**: 用 AI stub 生成 review.json, 或直接复制用户上传的
...
1. **AI stub 不是真实评审** — `api/ai_stub.py` 产的 review.json 是全 3 分占位, 必须接真实 LLM 才是 MVP
2. **没有用户系统** — 单用户 / 无鉴权, 跑在本机
3. **没有持久队列** — 服务重启运行中的 job 不会自动恢复 (但历史记录从磁盘能恢复)
4. **没有自动抓取** — Phase 3 要用户手动上传 `raw_assets.zip`, Phase 4 才做 yt-dlp / 商店爬虫
